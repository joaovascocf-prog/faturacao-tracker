...
app.get('/moloni-callback', async (req, res) => {
  ...
  try {
    ...
    if (data.access_token) {
      ...
      console.log("Tokens stored, company_id:", moloniTokens.company_id);
...
app.get('/moloni-status', async (req, res) => {
  try { 
    await getToken(); 
    res.json({ connected: true, company_id: moloniTokens.company_id }); 
  }
...
app.get('/moloni-companies', async (req, res) => {
  try {
    ...
    // If we have company_id from callback, return it directly
    if (moloniTokens.company_id) {
      return res.json([{ company_id: moloniTokens.company_id }]);
    }
...
// Dropbox file content proxy (CORS workaround)
app.post('/dropbox/read', async (req, res) => {
  ...
  try {
    ...
    if (r.status === 404 || r.status === 409) return res.json({});
    if (!r.ok) return res.status(r.status).json({ error: 'Dropbox error ' + r.status });
    const text = await r.text();
    try { res.json(JSON.parse(text)); } catch { res.json({}); }
  } catch(e) { res.status(500).json({ error: e.message }); }
});

app.post('/dropbox/write', async (req, res) => {
  const { token, path, data } = req.body || {};
  if (!token || !path) return res.status(400).json({ error: 'Missing params' });
  try {
...
app.listen(PORT, () => console.log('Server v12 running on port ' + PORT));
